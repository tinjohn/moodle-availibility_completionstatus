...
/**
 * Activity completion status condition.
 *
 * @package availability_completionstatus
 * @copyright 2014 The Open University
 * @copyright 2024 Tina John <tina.john@th-luebeck.de>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace availability_completionstatus;
...
class condition extends \core_availability\condition {
    ...
    public function __construct($structure) {
        ...
        if (isset($structure->cm) && is_number($structure->cm)) {
            ...
            throw new \coding_exception('Missing or invalid ->cm for completion status condition');
        }
        ...
        if (isset($structure->e) && in_array($structure->e,
            ...
            throw new \coding_exception('Missing or invalid ->e for completion status condition');
        }
    }
    ...
    private function load_course_structure(stdClass $course): void {
        ...
        if (empty($this->modfastprevious)) {
            $previouscache = cache::make('availability_completionstatus', 'previous_cache');
    ...
    public function get_description($full, $not, info $info): string {
        ...
        if ($this->cmid == self::OPTION_PREVIOUS && $editing) {
            ...
            $modinfo = $info->get_modinfo();
            if (!array_key_exists($cmid, $modinfo->cms) || $modinfo->cms[$cmid]->deletioninprogress) {
                $modname = get_string('missing', 'availability_completionstatus');
            } else {
        ...
        return get_string($str, 'availability_completionstatus', $modname);
    }
    ...
    public static function completion_value_used($course, $cmid): bool {
        ...
        if (!array_key_exists($course->id, self::$modsusedincondition)) {
            ...
            foreach ($modinfo->cms as $othercm) {
                ...
                foreach ($tree->get_all_children('availability_completionstatus\condition') as $cond) {
